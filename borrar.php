...
<body>
    ...
    <div class="table-container">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_Ruta'])) {
            ...
            $delete_query = "DELETE FROM ruta WHERE id_Ruta = $id_Ruta";
            if (mysqli_query($con, $delete_query)) {
                echo "<p class='success'>Ruta con ID $id_Ruta eliminada exitosamente.</p>";
            } else {
                echo "<p class='error'>Error al eliminar la ruta: " . mysqli_error($con) . "</p>";
            }
        }

        $resultado = mysqli_query($con, "SELECT * FROM ruta");

        echo "<table>
        <tr>
            <th>Id_Ruta</th>
            <th>Destino</th>
            <th>Material que maneja</th>
            <th>Acción</th>
        </tr>";
        ...
        while ($row = mysqli_fetch_array($resultado)) {
            ...
            echo "<td>" . $row['Material que maneja'] . "</td>";
            echo "<td>
                <form method='POST'>
